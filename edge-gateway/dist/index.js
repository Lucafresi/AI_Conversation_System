...
var ChatRequestSchema = external_exports.object({
  ...
  tools_wanted: external_exports.boolean().optional(),
  quality: external_exports.enum(["auto", "max", "cost_optimized"]).default("auto"),
  stream: external_exports.boolean().default(true)
});
var AuthHeaderSchema = external_exports.object({
  authorization: external_exports.string().regex(/^Bearer .+/)
});
var router = e();
var corsMiddleware = (request) => {
...
var rateLimitMiddleware = async (request, env) => {
  ...
  try {
    ...
    if (current) {
      ...
      if (Date.now() - timestamp < window * 1e3) {
        if (count >= limit) {
...
var authMiddleware = async (request, env) => {
  try {
...
router.post("/rag/query", async (request, env) => {
  try {
    const body = await request.json();
